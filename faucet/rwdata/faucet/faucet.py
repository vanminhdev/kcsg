...
from faucet.process_file_queue import write_txt_file, ProcessFileQueue
...
class SinaApiSample(ControllerBase):
    ...
    @route("update_version", url + '/versions/update-version', methods=['POST'])
    def update_version(self, req):
        data = json.loads((b'' + req.body).decode())
        Kcs.set_version(data["ip"], data["version"])

    @route("get_version", url + '/versions/get-version', methods=['POST'])

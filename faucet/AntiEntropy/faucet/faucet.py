...
class Faucet(RyuAppBase):
    ...
    def send_notify(self, event_info):
        self.logger.info('SENDING NOTIFY...')
        print(event_info)

        try:
            kind = event_info['event_type']
            data = event_info['data']
            data = json.dumps(data)

            info = self.get_ip_communication()
            local_ip = info['localIp']
            send_data = "{\"ip\": \"" + local_ip + "\", \"kind\": \"" + kind + "\", \"data\": " + data + "}"
            ...
            for i in range(count):
                ...
                if 'ip' in event_info and event_info['ip'] != ip:
                    continue
                ...
                try:
                    if controller == "ONOS":
                        ...
                        self.update_handled_file(event_info['id'], ip, '1')
                        print("Send notify success | ONOS")
                    ...
                    print('error: ')
                    print(e)
                    self.update_handled_file(event_info['id'], ip, '0')
